 ...
 ////  Umpire is enabled if UMPIRE_ENABLE_CUDA is defined
 ////  GPU funcition for BOUT++ 
 ////  Yining Qin update 0730-2020 - 0910-2020, Qin3@llnl.gov, Lawrence Livermore National Laboratory
...
#include "umpire/Allocator.hpp" // using Umpire lib
#include "umpire/ResourceManager.hpp" // using Umpier lib
...
// Umpire structure of Unified Memory
namespace {
template <typename G>
using itr = G*;
template <typename G>
...
// Raw pointers to copy data to GPU.
 ...
 __managed__ BoutReal r_gpu_DZZ_par = 0.00;
...
// gpu version hybrid model
template<CELL_LOC location>
static BoutReal RAJA_DEVICE gpu_Delp2_par(const FieldAccessor<location> &g, const int i) {
...
// gpu version hybrid model
template<CELL_LOC location>
  static BoutReal RAJA_DEVICE gpu_bracket_par(const FieldAccessor<location> &f, const int i) {
	Coordinates *metric = g.coords;
  BoutReal dz = metric->dz;
...
// gpu version hybrid model
template< CELL_LOC location>
static BoutReal RAJA_DEVICE  gpu_DZZ_par(const FieldAccessor<location> &g, const int i) {
   Coordinates *metric = g.coords;
   BoutReal dz = metric->dz;
...
static BoutReal RAJA_DEVICE  gpu_Div_par_Grad_par(const FieldAccessor<location> &f, const int i) {  
  ...
  BoutReal dz = metric->dz;
  auto div_p =  gpu_Div_par_Grad_par_G(i, p_phi_minus_n,  p_g22, dxMesh,dyMesh,dz,JMesh,nxMesh, nyMesh, nzMesh,phi_minus_n_acc_yup, phi_minus_n_acc_ydown);
  return div_p;
     }
